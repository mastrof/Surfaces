...
for interaction in ["stick"], mot in ["RunTumble", "RunReverse", "RunReverseFlick"],
    ...
    plot(palette=cmap, legendtitle=L"\mathrm{U\tau \, / \, W}",
        ...
        xlab=L"\mathrm{length \; of \; free \; path \, / \, run \; length}",
    ...
    savefig(plotsdir(savename("relativefreepath", config, "png")))
    savefig(plotsdir(savename("relativefreepath", config, "pdf")))
end
